function Monster:onDropLoot(corpse)
    ...
    if not player or player:getStamina() > 840 then
        ...
        for i = 1, #monsterLoot do
            ...
            if not item then
                Spdlog.info("Could not add loot item to corpse of monster '" ..
                                mType:getName() .. "'")
            end
        end
        ...
        if player then
            ...
            if self:getName():lower() == (Game.getBoostedCreature()):lower() then
                text = ("Loot of %s: %s (boosted loot)"):format(
                           mType:getNameDescription(),
                           corpse:getContentDescription())
                if party or version < 1200 then
                    oldClientText = ("Loot of %s: %s (boosted loot)"):format(
                                        mType:getNameDescription(),
                                        corpse:getContentDescription(true))
                end
            else
                text = ("Loot of %s: %s"):format(mType:getNameDescription(),
                                                 corpse:getContentDescription())
                if party or version < 1200 then
                    oldClientText = ("Loot of %s: %s"):format(
                                        mType:getNameDescription(),
                                        corpse:getContentDescription(true))
                end
            end
